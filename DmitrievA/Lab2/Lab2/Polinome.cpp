...
Polinom Polinom::operator+(const Polinom& other) const {
  ...
  for (; it != monomes.cend() && it2 != other.monomes.cend();) {
    if (it->degree == it2->degree) {
      ...
      if (abs(coeff) > EPS)
        result.monomes.push_back(Monome(coeff, it->degree));
      it++;
      it2++;
    } else if (*it < *it2) {
  ...
  if (polinom.parseMode == Simple) {
    ...
    for (size_t i = 0; i < line.size(); i++) {
      ...
      if (line[i] == '^') {
        ...
        if (x) {
          throw "Double x at position " + to_string(i);
          return in;
        }
        x = true;
        ix = 1;
        ...
        if (y) {
          throw "Double y at position " + to_string(i);
          return in;
        }
        y = true;
        iy = 1;
        ...
        if (z) {
          throw "Double z at position " + to_string(i);
          return in;
        }
        z = true;
        iz = 1;
...
Monome Monome::operator*(const Monome& other) const {
  ...
  if (this->degree % 10 + other.degree % 10 > 9 ||
      this->degree % 100 / 10 + other.degree % 100 / 10 > 9 ||
      this->degree / 100 + other.degree / 100 > 9) {
