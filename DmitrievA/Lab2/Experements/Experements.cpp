...
void runExperiment(const string& tableName, const vector<string>& keys, const vector<Polinom>& values) {
    TTable table;
    size_t n = keys.size();
    ...
    cout << left << setw(20) << tableName
        << " | Insert: " << setw(10) << insertRes.totalOps << " ops / " << setw(8) << fixed << setprecision(2) << insertRes.durationMs << " ms"
        << " | Find: " << setw(10) << findRes.totalOps << " ops / " << setw(8) << findRes.durationMs << " ms"
        << " | Remove: " << setw(10) << removeRes.totalOps << " ops / " << setw(8) << removeRes.durationMs << " ms" << endl;
}
